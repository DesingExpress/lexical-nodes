  ...
  const onActiveChange = React.useCallback(
    ({ activeItems }) => {
      ...
      if (activeItems.length === 1) {
        ...
        setDropdownLabel(
          i18n.t("lexical:general:toolbarItemsActive", {
  ...
  return (
    ...
    >
      ...
      {group.type === "buttons" && group.items.length
        ...
        : null}
      {index < editorConfig.features.toolbarFixed?.groups.length - 1 && (
...
const getParentEditorWithFixedToolbar = (editorConfigContext) => {
  if (editorConfigContext.parentEditor?.editorConfig) {
    if (
      editorConfigContext.parentEditor?.editorConfig.resolvedFeatureMap.has(
        "toolbarFixed"
      )
    ) {
      ...
      if (editorConfigContext.parentEditor) {
        return getParentEditorWithFixedToolbar(
          editorConfigContext.parentEditor
        );
      }
    }
  }
...
export const FixedToolbarPlugin = ({ clientProps }) => {
  ...
  const parentWithFixedToolbar =
    getParentEditorWithFixedToolbar(editorConfigContext);
