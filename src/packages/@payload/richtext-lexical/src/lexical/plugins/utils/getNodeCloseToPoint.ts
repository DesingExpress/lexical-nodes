...
import { getBoundingClientRectWithoutTransform } from "../handles/DraggableBlockPlugin/getBoundingRectWithoutTransform.js";
import { getCollapsedMargins } from "./getCollapsedMargins.js";
import { getTopLevelNodeKeys } from "./getTopLevelNodeKeys.js";
...
export function getNodeCloseToPoint(props: Props): Output {
  ...
  editor.getEditorState().read(() => {
    if (useEdgeAsDefault) {
      ...
      if (firstNode && lastNode) {
        ...
        if (y < firstNodeRect.top) {
          ...
          closestBlockElem.blockNode = $getNodeByKey(
    ...
    while (index >= 0 && index < topLevelNodeKeys.length) {
      ...
      const domRect = Rect.fromDOMRect(
        getBoundingClientRectWithoutTransform(elem)
      );
      ...
      const { distance, isOnBottomSide, isOnTopSide } =
