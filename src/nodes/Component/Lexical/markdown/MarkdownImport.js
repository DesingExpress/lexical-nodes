...
import { IS_APPLE_WEBKIT, IS_IOS, IS_SAFARI } from "../utils/environment.js";
...
function $importMultiline(lines, startLineIndex, multilineElementTransformers, rootNode) {
    for (const transformer of multilineElementTransformers) {
        ...
        while (endLineIndex < linesLength) {
            ...
            if (replace(rootNode, null, startMatch, endMatch, linesInBetween, true) !==
                false) {
...
function createTextFormatTransformersIndex(textTransformers) {
    ...
    return {
        ...
        openTagsRegExp: new RegExp((IS_SAFARI || IS_IOS || IS_APPLE_WEBKIT ? "" : `${escapeRegExp}`) +
            "(" +
            openTagsRegExp.join("|") +
            ")", "g"),
        transformersByTag,
